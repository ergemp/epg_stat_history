...
declare 
  ...
  row_data RECORD;
begin
	CREATE TEMP TABLE IF NOT EXISTS temp_results_get_series_archiver_hist (
		begin_ts bigint, 
		end_ts bigint, 
  ...
  open c1 for
    ...
    LOOP
      ...
      insert into temp_results_get_series_archiver_hist
        select * from epg_stats.get_stat_archiver_hist(row_data.current_snapshot, (to_timestamp(row_data.current_snapshot)-to_timestamp(row_data.previous_snapshot))::interval);    
    END LOOP;
    CLOSE c1;

	return query
	select * from temp_results_get_series_archiver_hist;
	
	DROP TABLE IF EXISTS temp_results_get_series_archiver_hist;
end;
$function$
;